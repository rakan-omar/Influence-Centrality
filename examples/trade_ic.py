...
Selected_Countries = ["USA", "CHN", "DEU", "GBR", "FRA", "NLD", "HKG", "JPN", "ITA", "CAN"] #using the listed top 10 importers (CA is 10th which works out well)
...
def draw_G(G, filename = 'network_graph'):
    ...
    graph = Network('700px', '1300px', directed=True)
    graph.from_nx(G)
    graph.set_edge_smooth("dynamic")
    for edge in graph.edges:
        edge['label'] = round(edge['weight'], 4)
...
for row in reader:
    ...
    if (row[2] in Selected_Countries and row[9] in Selected_Countries): #if data for 'world' is in the file, don't include it.
        ...
        if (arrow_from != arrow_to): #found two loops, plus the one on world, throwing off the calculcations
            ...
            #if edge already exists, add total_trade_value to previous weight.
...
for row in read:
    if (row[1] in Selected_Countries):
        ...
        G.nodes[row[1]]['name'] = row[0]
        if GDP > max_gdp:
            max_gdp = GDP
        #if GDP < min_gdp:
        #    min_gdp = GDP
    #elif (row[1] != "WLD"): #not in North_america and not world, include in rotw
        #old_value = 0
        #try:
...
for edge in G.edges:
    ...
    edge_weight = G.get_edge_data(edge[0], edge[1])['weight'] * 1000 #imports exports listed in thousands of dollars. Gdp in dollars.
